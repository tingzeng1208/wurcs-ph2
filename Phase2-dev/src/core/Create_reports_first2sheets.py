...
class CreateReports:
    ...
    def write_titles_and_column_headers(self, ws, dtTitles, sSelectWorktable, sTitle_RR_YEAR, sTitle_WORKTABLE, iColumnCount, sSheetTitle):
        ...
        for _, drTitles in dtTitles[(dtTitles["worktable"] == sSheetTitle[:2]) & (dtTitles["part"] == sSheetTitle[sSheetTitle.index('P')+1:sSheetTitle.index('P')+3])].iterrows():
            # Worktable titles
            ws.cell(row=3, column=2, value=str(drTitles["title1"]))
            ws.cell(row=3, column=2).font = Font(bold=True)
            ws.cell(row=4, column=2, value=str(drTitles["title2"]))
            ws.cell(row=4, column=2).font = Font(bold=True)
            ws.cell(row=5, column=2, value=str(drTitles["title3"]))
            ...
            iROW_COUNT += 2
            # Write footnotes
            for drRow in self.dtFootnotes[(self.dtFootnotes["worktable"] == sSheetTitle[:2]) & (self.dtFootnotes["part"] == sSheetTitle[sSheetTitle.index('P')+1:sSheetTitle.index('P')+3])].itertuples(index=False):
