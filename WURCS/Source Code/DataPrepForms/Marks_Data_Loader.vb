...
Public Class Marks_Data_Loader
    ...
    Private Sub Marks_Data_Loader_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub btn_Execute_Click(sender As Object, e As EventArgs) Handles btn_Execute.Click
        ...
        Dim mThisRoadMark As String, mThisMaintStamp As String
        ...
        If bolWrite = vbYes Then
            ...
            Gbl_Marks_Tablename = Get_Table_Name_From_SQL("1", "MARKS")

            ' open the connection to the SQL server
            Gbl_Waybill_Database_Name = "WAYBILLS"
            OpenSQLConnection(Gbl_Waybill_Database_Name)

            'If the table does not exist, create it
            If VerifyTableExist(Gbl_Waybill_Database_Name, Gbl_Marks_Tablename) = False Then
                Create_Marks_Table()
            End If
            ...
            Do While Not mSr.EndOfStream
                ...
                'Remove any apostrophe's from the record (primarily for non-US companies)
                mInString = Replace(mInString, "'", " ")
                'Get the index key values for this table
                mThisRoadMark = Trim(mInString.Substring(0, 4))
                mThisMaintStamp = Trim(mInString.Substring(162, 26))

                'Check to see if this record exists
                If Count_Marks_Records(mThisRoadMark, mThisMaintStamp) = 0 Then
                    ' This record doesn't exist. We need to insert it
                    mSQLStr = Build_Insert_Marks_Record(mInString, Gbl_Marks_Tablename)
                Else
                    ' A record already exists.  We need to update it
                    mSQLStr = Build_Update_Marks_Record(mInString, Gbl_Marks_Tablename)
                End If
